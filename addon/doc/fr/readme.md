...
| **2026.2** | **Défilement automatique braille** intégré (curseur de vitesse dans les paramètres Braille et commande NVDA associée). Braille Extender propose un **défilement automatique** plus riche ; voir [Défilement automatique](#défilement-automatique). |
...
| **Défilement automatique** | Délais et comportement du **défilement automatique braille de Braille Extender** (voir [Défilement automatique](#défilement-automatique) ; NVDA 2026.2+ propose aussi un défilement automatique intégré). |
...
Braille Extender propose son propre **défilement automatique braille** (sur minuteur, avec délai par afficheur et options ci-dessous). Utilisez l’onglet **Braille Extender → Défilement automatique** et la commande **défilement automatique** dans **Touches de commandes** (les profils d’afficheur peuvent la lier).

À partir de **NVDA 2026.2**, NVDA inclut aussi un **défilement automatique** distinct dans **Paramètres NVDA → Braille** (curseur de vitesse et commande **Basculer le défilement automatique braille** dans les commandes braille NVDA). Les deux fonctions **ne sont pas équivalentes** : Braille Extender ajoute un **délai par modèle d’afficheur**, **l’ajustement au contenu**, **l’ignorance des lignes vides** et des **touches plus rapide / plus lent** pour le délai. Tant que le **défilement automatique Braille Extender** est activé, l’extension **désactive le défilement automatique intégré de NVDA** pour éviter les conflits. Pour le comportement le plus complet, utilisez le **défilement automatique Braille Extender** ; n’utilisez celui de NVDA que si vous préférez son défilement plus simple basé sur la vitesse et que vous avez désactivé celui de l’extension.
...
- **Défilement automatique** avec délai par afficheur, options lignes vides et coordination avec le défilement automatique intégré de NVDA 2026.2+.
